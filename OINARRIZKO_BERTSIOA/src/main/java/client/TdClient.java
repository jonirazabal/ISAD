...
public final class TdClient implements Runnable {
    ...
    private final Client.ResultHandler mDefaultHandler;


	private TdClient(ViewControl control, Path appDir) {
		mControl = control;
	...
	public static TdClient create(ViewControl control, Path appDir) {

		// TODO: multiple client allowed?
		TdClient client = new TdClient(control, appDir);
	   ...
	   // private static Client client = null;
	    ...
	    public String getFirstChat() {
            ...
            client.send(new TdApi.GetChat(getChatId(id)), defaultHandler);
            TdApi.Chat chat = chats.get(chatId);
    		TdApi.Message lastMessage = chat.lastMessage;
    		TdApi.MessageText content = (TdApi.MessageText) lastMessage.content;
	    ...
	    public void getChatMessages(Long mID, Long last, int howmany) {
			if (howmany > 0) {
				client.send(new TdApi.GetChatHistory(mID /* chatId */, last /* messageID */, 0 /* offset */,
						howmany /* limit */, false /* onlyLocal */), new Client.ResultHandler() {
						@Override
						public void onResult(TdApi.Object object) {
							...
							Arrays.asList(messages).stream().forEach(TdClient.this::parseContent);
							Long fromMessageId = messages[messages.length - 1].id;
							TdClient.this.getChatMessages(mID, fromMessageId, 
	    ...
	    public void getChatList(final int limit) {
	        synchronized (chatList) {
	            ...
	            for (int i = 0; i < limit; i++) {
	                ...
	                TdApi.Chat chat = chats.get(chatId);
	                synchronized (chat) {
	                	mControl.gehituTxatIzenburua(chatId, chat.title, chat.lastMessage.date); 
	                    System.out.println(chatId + ": " + chat.title + " Unread:(" + chat.unreadCount + ")" );
	                }
	            }
	            print("");
	        }
	        ...
	        public void onResult(TdApi.Object object) {
	            ...
	            switch ( object.getConstructor() ) {
	            	case TdApi.Chat.CONSTRUCTOR:
	            		...
	            		TdApi.Message lastMessage = chatObject.lastMessage;
	            		parseContent(lastMessage);
	            }
	        }
	    }
	    public void parseContent(TdApi.Message message) {
	    	if (message.content instanceof TdApi.MessagePhoto) {
				int m=0;
				TdApi.Photo photo = ((TdApi.MessagePhoto) message.content).photo;
				TdApi.PhotoSize size[] = photo.sizes;
				for(int i=0;i<=size.length-1;i++) {
					TdApi.PhotoSize ps = size[i];
					if(ps.type.equals("m")) {
						m=i;
					}
				}
				TdApi.File photoM = size[m].photo;
				int id = photoM.id;
				downloadFile(id);
        		String path = photoM.local.path;
				String caption = ((TdApi.MessagePhoto) message.content).caption.text;
				mControl.bistaratuIrudia(id, path);
				mControl.bistaratuMezua(caption);
				
	    	}
			else if(message.content instanceof TdApi.MessageText) {
				TdApi.MessageText content = (TdApi.MessageText) message.content;
	    		String text = content.text.text;
	    		mControl.bistaratuMezua(text);
    		}
	    }
	    
	    public void downloadFile(int fileId) {
	    	TdApi.DownloadFile download = new TdApi.DownloadFile(fileId,20);
	    	client.send(download, defaultHandler);
	    }
	        ...
	        public void onResult(TdApi.Object object) {
	            switch (object.getConstructor()) {
	            	case TdApi.UpdateFile.CONSTRUCTOR:
	    				...
	    				String caption = ((TdApi.MessagePhoto) object).caption.text;
	    				mControl.bistaratuIrudia(id, path);
	    				mControl.bistaratuMezua(caption);*/
